#include "runtime/function/render/passes/practice_pass.h"

#include "runtime/function/render/interface/vulkan/vulkan_rhi.h"
#include "runtime/function/render/interface/vulkan/vulkan_util.h"

#include <post_process_vert.h>
#include <practice_frag.h>

#include <stdexcept>

namespace Piccolo
{
    void PracticePass::initialize(const RenderPassInitInfo* init_info)
    {
        RenderPass::initialize(nullptr);

        const PracticePassInitInfo* _init_info = static_cast<const PracticePassInitInfo*>(init_info);
    ...
    void PracticePass::setupDescriptorSetLayout()
    {
        m_descriptor_infos.resize(1);

        RHIDescriptorSetLayoutBinding post_process_global_layout_bindings[1] = {};

        RHIDescriptorSetLayoutBinding& color_attachment_binding = post_process_global_layout_bindings[0];
        color_attachment_binding.binding         = 0;
        color_attachment_binding.descriptorType  = RHI_DESCRIPTOR_TYPE_INPUT_ATTACHMENT;
        color_attachment_binding.descriptorCount = 1;
        color_attachment_binding.stageFlags      = RHI_SHADER_STAGE_FRAGMENT_BIT;

        // RHIDescriptorSetLayoutBinding& normal_attachment_binding = post_process_global_layout_bindings[1];
        // color_attachment_binding.binding         = 1;
        // color_attachment_binding.descriptorType  = RHI_DESCRIPTOR_TYPE_INPUT_ATTACHMENT;
        // color_attachment_binding.descriptorCount = 1;
        // color_attachment_binding.stageFlags      = RHI_SHADER_STAGE_FRAGMENT_BIT;
            
        // RHIDescriptorSetLayoutBinding& depth_attachment_binding = post_process_global_layout_bindings[2];
        // color_attachment_binding.binding         = 2;
        // color_attachment_binding.descriptorType  = RHI_DESCRIPTOR_TYPE_INPUT_ATTACHMENT;
        // color_attachment_binding.descriptorCount = 1;
        // color_attachment_binding.stageFlags      = RHI_SHADER_STAGE_FRAGMENT_BIT;
    ...
    void PracticePass::setupPipelines()
    {
        ...
        RHIShader* frag_shader_module = m_rhi->createShaderModule(PRACTICE_FRAG);
        ...
        pipelineInfo.subpass             = _main_camera_subpass_practice;
    ...
    void PracticePass::setupDescriptorSet()
    {
    ...
    void PracticePass::updateAfterFramebufferRecreate(RHIImageView* input_attachment)
    {
        std::array<RHIDescriptorImageInfo, 1> post_process_per_frame_input_attachment_info{};
        post_process_per_frame_input_attachment_info[0].sampler     = nullptr;
        post_process_per_frame_input_attachment_info[0].imageView   = input_attachment;
        post_process_per_frame_input_attachment_info[0].imageLayout = RHI_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

        // post_process_per_frame_input_attachment_info[1].sampler     = nullptr;
        // post_process_per_frame_input_attachment_info[1].imageView   = input_normal_attachment;
        // post_process_per_frame_input_attachment_info[1].imageLayout = RHI_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        
        // post_process_per_frame_input_attachment_info[2].sampler     = nullptr;
        // post_process_per_frame_input_attachment_info[2].imageView   = input_depth_attachment;
        // post_process_per_frame_input_attachment_info[2].imageLayout = RHI_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;


        RHIWriteDescriptorSet post_process_descriptor_writes_info[1];

        for (size_t i = 0; i < 1; ++i) {
            post_process_descriptor_writes_info[i].sType = RHI_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
            post_process_descriptor_writes_info[i].pNext = NULL;
            post_process_descriptor_writes_info[i].dstSet = m_descriptor_infos[0].descriptor_set;
            post_process_descriptor_writes_info[i].dstBinding      = 0;
            post_process_descriptor_writes_info[i].dstArrayElement = 0;
            post_process_descriptor_writes_info[i].descriptorType  = RHI_DESCRIPTOR_TYPE_INPUT_ATTACHMENT;
            post_process_descriptor_writes_info[i].descriptorCount = 1;
            post_process_descriptor_writes_info[i].pImageInfo = &post_process_per_frame_input_attachment_info[i];
        }
    ...
    void PracticePass::draw()
    {
        float color[4] = { 1.0f, 1.0f, 1.0f, 1.0f };
        m_rhi->pushEvent(m_rhi->getCurrentCommandBuffer(), "Practice subpass", color);
